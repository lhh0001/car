...
> 本文是当前接线的唯一参考（接线方案 v2）。旧版 USB 串口架构说明保留在
...
> 旧的示意图片仍在仓库中，但其中左编码器使用 GPIO34/35，已经废弃；请完全以本文的文字表格接线。
...
电机电池 ──> 独立 5V 降压模块 ──> ESP32 VIN/5V 输入、LiDAR VCC
...
| `TX` | 经 5V→3.3V 电平转换后接 ESP32 `GPIO16` | 这是“雷达发、ESP32 收”。GPIO16 已启用内部上拉，可稳定开漏或弱驱动的雷达 TX 高电平。**确认雷达 TX 是5V TTL时，绝不能直接接GPIO16。** |
| `RX` | ESP32 `GPIO17`，必要时先经 3.3V→5V 电平转换 | 这是“ESP32 发、雷达收”。当前实测普通 UART 推挽输出能触发雷达开始发送数据。 |
...
**不要从 L298N 板的 `5V` 引脚给 ESP32 或雷达供电。** 常见 L298N 模块的板载稳压器余量很小，电机一启动就可能让 ESP32 掉压重启。应使用电池后单独的 5V、至少 3A 的降压模块。若你的 L298N 有 `5V_EN` 跳帽：使用独立 5V 供电时先拔掉该跳帽，再把降压模块的 5V 接到 L298N `5V` 逻辑端（以板上丝印为准）。
...
这组 N20 电机的红白两根是**电机本体**，只能接 L298N 输出；黑蓝黄绿四根是编码器。若编码器规格写着支持 `3.3–5V`，优先以 3.3V 供电，这样黄绿信号可以直接进 ESP32。若它明确只能 5V 供电，黄绿两路必须先各经过 5V→3.3V 电平转换。

| N20 线色 / 信号 | 应接到哪里 |
|---|---|
| 红：电机 `+` | 左电机接 L298N `OUT1`；右电机接 `OUT3` |
| 白：电机 `-` | 左电机接 L298N `OUT2`；右电机接 `OUT4` |
| 黑：编码器 VCC | ESP32 `3V3`（仅限编码器支持 3.3V 时） |
| 蓝：编码器 GND | 公共 `GND` |
| 黄：A 相 | 左轮接 `GPIO13`；右轮接 `GPIO18` |
| 绿：B 相 | 左轮接 `GPIO23`；右轮接 `GPIO19` |
...
| 右轮 B 相 | `GPIO19` |
| 编码器 GND | 公共 GND |
| 编码器 VCC | `3V3`（前提：编码器规格允许 3.3V） |

```text
左编码器 A（黄）─ ESP32 GPIO13  右编码器 A（黄）─ ESP32 GPIO18
左编码器 B（绿）─ ESP32 GPIO23  右编码器 B（绿）─ ESP32 GPIO19
左编码器 GND ─ 公共 GND         右编码器 GND ─ 公共 GND
左/右编码器 VCC（黑）─ ESP32 3V3
```

注意：

- ESP32 GPIO 不能直接接受 5V。若编码器必须以 5V 供电且 A/B 输出为 5V，A/B 各路都需要 5V→3.3V 电平转换。
- 这版将左轮移至 GPIO13/GPIO23，避免 GPIO34/GPIO35 没有内部上拉的问题。若编码器是开漏/开集电极，即使已有内部上拉，也建议 A/B 各加一个 **4.7–10 kΩ** 外部上拉到 `3V3`，抗干扰更强。
...
ESP32 3V3 ───────────────────────> GY-521 VCC
ESP32 3V3 ───────────────────────> 两个编码器黑线（仅编码器支持 3.3V）
```

`*` 只适用于你拔掉 L298N 的 `5V_EN` 跳帽、并确认该端是逻辑 5V 输入的情况；不同模块丝印不同，以模块说明为准。
...
实车运动参数统一为：轮子直径 `0.04 m`（半径 `0.02 m`）、左右轮中心距
`0.13 m`、轮端每圈 `360` 个 A 相上升沿计数。实测左轮约 `357.8`、
右轮约 `359.0` 次/圈；软件将右轮原始计数乘以 `-1`，使车辆前进时
左右里程均为正。编码器报文为：

```text
E <boot_id> <sequence> <sample_ms> <left_count> <right_count>
```

`boot_id` 用于识别 ESP32 重启，`sequence` 用于丢弃重复或乱序 UDP 包，
`sample_ms` 用于按 ESP32 的采样时钟计算轮速，避免 Wi-Fi 包成批到达时产生速度尖峰。

左编码器改脚后必须重新烧录固件。接线完成后的第一步不是立刻建图，而是先分别验证：雷达是否有 `/scan`、IMU 是否有 `/imu`、手推车轮时 `/odom` 是否有变化。确认三项都正确后再启动实车建图。
...
关闭 `wifi_bridge` 和其他电机测试后，运行：

```bash
python3 src/vehicle_hardware/scripts/esp32_encoder_monitor.py --duration 15
```

它只发送停车 `M 0 0`，兼容新版带启动 ID、序号和采样时间的 `E` 报文，也兼容旧版 `E <left_count> <right_count>`；屏幕会显示左右累计计数及两次回传之间的 `d_left` / `d_right`。手转左轮应看到左侧计数变化；手转右轮应看到右侧计数变化。
