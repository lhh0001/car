...
这组 N20 电机的红白两根是**电机本体**，只能接 L298N 输出；黑蓝黄绿五根是编码器。若编码器规格写着支持 `3.3–5V`，优先以 3.3V 供电，这样黄绿信号可以直接进 ESP32。若它明确只能 5V 供电，黄绿两路必须先各经过 5V→3.3V 电平转换。
...
它只发送停车 `M 0 0`，屏幕会显示 ESP32 回传的 `E <left_count> <right_count>`、两次回传之间的 `d_left` / `d_right`。手转左轮应看到左侧计数变化；手转右轮应看到右侧计数变化。
