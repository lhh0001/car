...
public:
        ...
        : Node("lidar_node"), fd_(-1), is_shutdown_(false), scan_count_(0), last_point_angle_(0.0)
    {
        ...
        if (transport_ == "tcp") {
            ...
            RCLCPP_FATAL(this->get_logger(), "Unknown transport '%s' (use tcp or serial)", transport_.c_str());
        }
        if (!opened) {
...
private:
    ...
    bool openTcp(const std::string& host, int port)
    {
        ...
        if (connect(fd_, reinterpret_cast<sockaddr*>(&address), sizeof(address)) != 0) {
            RCLCPP_ERROR(this->get_logger(), "TCP connection to %s:%d failed: %s",
        ...
        if (flags == -1 || fcntl(fd_, F_SETFL, flags | O_NONBLOCK) == -1) {
            RCLCPP_ERROR(this->get_logger(), "Failed to set LiDAR TCP socket non-blocking: %s", strerror(errno));
            close(fd_);
            fd_ = -1;
            return false;
        }
